...
- prediction export (top-k MeSH + cosine similarity)
...
def stablerep_loss(
    ...
    logits = cosine_sim(z_g, z_m) / tau

    pos_mask = build_pos_mask(group)
...
@torch.no_grad()
def predict_topk_gly_to_mesh(
    proj_g: nn.Module,
    proj_m: nn.Module,
    G: torch.Tensor,
    M: torch.Tensor,
    gly_indices: List[int] | Iterable[int],
    gly_ids: List[str],
    mesh_ids: List[str],
    g2true: Optional[Dict[int, set]],
    device: torch.device,
    batch_size: int = 1024,
    topk: int = 30,
) -> pd.DataFrame:
    """
    Export per-glycan top-k predictions and cosine similarity scores.
    ...
    gly_indices = [int(x) for x in gly_indices]
    topk = int(topk)
    if topk <= 0:
    ...
    zmT = zm_all.t().contiguous()
    n_mesh = zm_all.size(0)
    k_eff = min(topk, n_mesh)

    rows: List[Dict[str, object]] = []

    for g_idx in gly_indices:
        scores = (zg_all[g_idx: g_idx + 1] @ zmT).squeeze(0)
        top_scores, top_indices = torch.topk(scores, k=k_eff)

        true_set = set(g2true.get(g_idx, set())) if g2true is not None else set()
        true_mesh_ids = ";".join(mesh_ids[m] for m in sorted(true_set)) if true_set else ""

        for rank, (m_idx, score) in enumerate(zip(top_indices.tolist(), top_scores.tolist()), start=1):
            rows.append({
                "glycan_row": int(g_idx),
                "glycan_id": str(gly_ids[g_idx]),
                "rank": int(rank),
                "mesh_row": int(m_idx),
                "mesh_id": str(mesh_ids[m_idx]),
                "score": float(score),
                "is_true": int(m_idx in true_set),
                "n_true_labels": int(len(true_set)),
                "true_mesh_ids": true_mesh_ids,
            })
...
def build_pairs_from_csv(
    ...
    pairs_csv row format:
      gly_id_col: glycan id
      mesh_list_col: semicolon-separated mesh IDs
...
__all__ = [
    ...
    "eval_retrieval_gly_to_mesh", "predict_topk_gly_to_mesh",
    "_train_one_epoch", "_save_ckpt", "_load_ckpt", "_parse_metric_k", "run_stage",
    "load_embeddings_from_csv", "build_pairs_from_csv",
    "make_kfold_assignment_on_unique_glycans", "load_preassigned_folds",
]