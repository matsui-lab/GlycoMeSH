...
Strategy A (holdout TEST + DEV CV + Optuna):
  1) Fix a glycan-level TEST split ONCE.
  2) On DEV, run Optuna where objective = 5-fold CV mean(metric).
  3) Pick best HP.
  4) Retrain on ALL DEV with best HP (two-stage, no val / no early-stop by val).
  5) Final evaluation on fixed TEST.

This version broadens the Optuna search space relative to the narrow local SAPBERT search.
...
- During Optuna objective, DEV 5-fold CV is run for each trial.
- Final DEV retrain uses NO validation and does not peek at TEST until the end.
- The commented example at the bottom uses a NEW study_dir so it does not overwrite prior outputs.
"""
...
def _coerce_trial_attrs_for_json(obj: Any) -> Any:
    """Recursively coerce numpy / Path-like / non-JSON-native types into JSON-safe values."""
    if isinstance(obj, dict):
        return {str(k): _coerce_trial_attrs_for_json(v) for k, v in obj.items()}
    if isinstance(obj, (list, tuple)):
        return [_coerce_trial_attrs_for_json(v) for v in obj]
...
def _run_dev_cv5_once(
    *,
    ...
    Objective value = mean(metric across folds).
    """
    set_seed(cfg.seed)
...
# ----------------------------
# Optuna search space (broader)
# ----------------------------
def suggest_sapbert_params_wide(trial: Trial) -> Dict[str, Any]:
    """
    Broader search than the narrow local SAPBERT search.

    Design principles:
      - keep stage1 LR > stage2 LR likely, but do not hard-force it
      - allow model capacity change (hidden / out_dim)
      - allow broader batch composition (batch_glycans / pos_per_glycan)
      - keep search ranges realistic enough for this task
    """
    out_dim = trial.suggest_categorical("out_dim", [256, 512, 768])
    hidden = trial.suggest_categorical("hidden", [0, 512, 1024, 1536])
    dropout = trial.suggest_float("dropout", 0.0, 0.30)

    tau = trial.suggest_float("tau", 0.02, 0.20, log=True)

    lr_stage1 = trial.suggest_float("lr_stage1", 1e-4, 2e-3, log=True)
    lr_stage2 = trial.suggest_float("lr_stage2", 5e-6, 5e-4, log=True)
    weight_decay = trial.suggest_float("weight_decay", 1e-6, 1e-2, log=True)

    batch_glycans = trial.suggest_categorical("batch_glycans", [128, 256, 512, 1024])
    pos_per_glycan = trial.suggest_categorical("pos_per_glycan", [2, 4, 8, 16])
...
def objective_factory(
    ...
    def objective(trial: Trial) -> float:
        ...
        hp = suggest_sapbert_params_wide(trial)
        ...
        params_out = {
            ...
            "params": _coerce_trial_attrs_for_json(hp),
            "fixed": {
                "epochs_stage1": int(args.epochs_stage1),
                "epochs_stage2": int(args.epochs_stage2),
                "n_folds": int(args.n_folds),
            ...
            "search_space": {
                "out_dim": [256, 512, 768],
                "hidden": [0, 512, 1024, 1536],
                "dropout": [0.0, 0.30],
                "tau": [0.02, 0.20],
                "lr_stage1": [1e-4, 2e-3],
                "lr_stage2": [5e-6, 5e-4],
                "weight_decay": [1e-6, 1e-2],
                "batch_glycans": [128, 256, 512, 1024],
                "pos_per_glycan": [2, 4, 8, 16],
            },
        }
        (trial_dir / "params.json").write_text(json.dumps(params_out, indent=2), encoding="utf-8")

        val, summary_df = _run_dev_cv5_once(
            cfg=cfg,
            G=G,
            M=M,
            dev_pairs=dev_pairs,
        ...
        trial.set_user_attr("n_completed_folds", int(len(summary_df)))
        return float(val)
...
def build_argparser():
    ...
    ap.add_argument("--study_name", default="sapbert_stablerep_holdout_test_cv5_wide")
    ...
    ap.add_argument("--cv_seed", type=int, default=42, help="Seed for DEV 5-fold split (fixed across trials/runs).")
...
def main():
    ...
    best_out = {
        ...
        "best_params": _coerce_trial_attrs_for_json(dict(study.best_params)),
        "mesh_embedding": "sapbert",
        "mesh_emb_csv": SAPBERT_MESH_EMB_CSV,
        "test_split": {
            "test_frac": float(args.test_frac),
...
# python optuna_sapbert_stablerep_holdout_test_cv5_wide.py \
#   --gly_emb_csv ./data/glycangt_embedding_2026/embeddings_large.csv \
#   --pairs_csv ./data/glycan/glytoucan_iupac_mesh_filtered.csv \
#   --study_dir ./data/analysis/contrastive_learning/optuna_sapbert_A_filtered_wide_20260413_run01 \
#   --study_name sapbert_stablerep_holdout_test_cv5_wide_run01 \
#   --n_trials 100 \
#   --metric recall@25 \
#   --test_frac 0.15 \
#   --early_stop \
