...
"""
optuna_glycan_mlp_holdout_test_cv5.py
====================================

Glycan-only baseline:
  glycan embedding -> MLP -> multi-label MeSH prediction
    ...
    *,
    ...
    for fold in fold_ids:
        ...
        s1_rows, s1_best, s1_best_epoch, s1_end_epoch, s1_best_path = run_stage(
            ...
            ckpt_name="best_multilabel_stage1.pth",
        )
        ...
        s2_rows, s2_best, s2_best_epoch, s2_end_epoch, s2_best_path = run_stage(
            ...
            min_delta=cfg.min_delta,
            warmup_epochs_global=cfg.warmup_epochs,
            ckpt_name="best_multilabel_stage2.pth",
        )
...
def _train_two_stage_no_val(
    *,
    ...
    ds_train = GlycanMultiLabelDataset(dev_gly)
    dl = torch.utils.data.DataLoader(
        ds_train,
        batch_size=cfg.batch_size,
        shuffle=True,
...
# Optuna search space
# ----------------------------
def suggest_params(trial: Trial) -> Dict:
    hidden1 = trial.suggest_categorical("hidden1", [512, 1024, 1536])
    hidden2 = trial.suggest_categorical("hidden2", [0, 512])
...
def objective_factory(
    ...
    def objective(trial: Trial) -> float:
        ...
        params_out = {
            "model": "glycan_only_multilabel_mlp",
...
def build_argparser():
    ...
    ap.add_argument("--study_name", default="glycan_mlp_multilabel_holdout_test_cv5")
...
def main():
    ...
    # full candidate MeSH vocabulary from mesh embedding CSV (same policy as CL)
    mesh_ids, mesh_id2row = load_mesh_ids_from_embedding_csv(args.mesh_emb_csv)

    # map supervision pairs onto the full candidate MeSH vocabulary
    ...
    Y, g2true_all = build_multilabel_targets(
        pairs=pairs,
        n_gly=G.size(0),
        n_mesh=len(mesh_ids),
    )
    ...
    objective = objective_factory(args, G, Y, dev_pairs, gly_ids, mesh_ids, row2fold_dev)
    ...
    # ----------------------------
    # Final retrain on all DEV, then TEST
    # ----------------------------
    print("\n[final] Retrain on ALL DEV with best HP (no val), then evaluate on fixed TEST.")

    bp = dict(study.best_params)
    final_cfg = TrainCfg(
        hidden1=int(bp["hidden1"]),
        hidden2=int(bp["hidden2"]),
    ...
    print("\n[done] Glycan-only multi-label baseline complete.")
