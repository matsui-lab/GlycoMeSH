...
"""
infer_all_topk_from_optuna_glycan_mlp_2nd.py
========================================

For each glycan, run the trained glycan-only multi-label MLP against the full
candidate MeSH space and write only TopK predictions.

Expected checkpoint format:
  final_dev_train/final_ckpt_stage2.pth
saved by the training script as:
  {
...
- Ranking by logit and ranking by sigmoid(prob) are identical because sigmoid
  is monotonic. We output both for convenience.
- Input glycan embedding normalization must match training. The training script
  default is L2-normalize unless --no_l2_norm was used. This inference script
  follows the same default behavior.

Example:
python infer_all_topk_from_optuna_glycan_mlp_2nd.py \
  --study_dir ./data/analysis/multilabel_glycan_only/optuna_holdout_cv5_fullmesh \
  ...
  --chunk_g 2048 \
  --device cuda \
  --out_csv ./data/analysis/multilabel_glycan_only/optuna_holdout_cv5_fullmesh/final_dev_train/infer_all_glycan_topk.csv
"""
...
def infer_topk_streaming(
    *,
    ...
    n_gly = G.size(0)
    n_mesh = len(mesh_ids)
    topk = max(1, min(int(topk), int(n_mesh)))
    ...
    for s in range(0, n_gly, chunk_g):
        ...
        x = G[s:e].to(device, non_blocking=True)  # (bs, d_in)
        logits = model(x)                         # (bs, n_mesh)

        topv, topi = torch.topk(logits, k=topk, dim=1, largest=True, sorted=True)

        if write_prob:
            topp = torch.sigmoid(topv)
            topp_cpu = topp.detach().cpu()
        else:
            topp_cpu = None
        ...
        for i in range(bs):
            ...
            if write_logit:
                logit_row = topv_cpu[i].tolist()
            if write_prob:
                prob_row = topp_cpu[i].tolist()
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        "--ckpt",
        default="",
        help="Optional checkpoint path. Default: <study_dir>/final_dev_train/final_ckpt_stage2.pth",
    )
    ...
    ap.add_argument("--save_meta_json", default="", help="Optional path to save inference metadata JSON.",)

    ap.add_argument("--no_logit", action="store_true", help="Do not write logit column.",)
    ap.add_argument("--no_prob", action="store_true", help="Do not write prob=sigmoid(logit) column.",)
...
def main():
    ...
    ckpt_path = (
        Path(args.ckpt)
        if args.ckpt.strip()
        else (study_dir / "final_dev_train" / "final_ckpt_stage2.pth")
    )
    ...
    # Load glycan embeddings in the same manner as training.
    G, gly_ids, gly_id2row = load_gly_embeddings_from_csv(
        args.gly_emb_csv,
        normalize=(not args.no_l2_norm),
    )

    # Load MeSH vocabulary order only.
    mesh_ids, mesh_id2row = load_mesh_ids_from_embedding_csv(args.mesh_emb_csv)
    ...
    if args.save_meta_json.strip():
        meta = {
            "script": "infer_all_topk_from_optuna_glycan_mlp.py",
